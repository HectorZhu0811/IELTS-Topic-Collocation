(function () {
  ...
  function exportMemory() {
    ...
    if (window.AndroidMemory && typeof window.AndroidMemory.saveMemory === "function") {
      window.AndroidMemory.saveMemory(payload, filename);
  ...
  function backupMemoryToAndroid() {
    if (!window.AndroidMemory || typeof window.AndroidMemory.backupMemory !== "function") return;
    try {
      window.AndroidMemory.backupMemory(JSON.stringify(memoryPayload()));
    } catch (error) {
      // Manual export remains available if native backup is blocked.
    }
  }
  ...
  function renderLanding() {
    ...
    topicLanding.innerHTML = `
      <div class="topic-reel-hero" aria-label="Topic reel selector">
        <button class="topic-memory-card" type="button" data-action="open-bank">Memory bank</button>
  ...
  function renderBankEntry() {
    ...
    collocationBank.innerHTML = `
      <div class="bank-compact-copy">
        ...
        <span>${due} due today · ${count} weak</span>
      </div>
      <button class="due-review-button" type="button" ${due === 0 ? "disabled" : ""}>今日复习 ${due}</button>
      <button class="bank-open compact-open" type="button" aria-label="Open collocation bank">${iconSvg("open")}<span>Open</span></button>
    `;
    collocationBank.querySelector(".due-review-button").addEventListener("click", () => {
