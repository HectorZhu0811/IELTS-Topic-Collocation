...
const fileUrl = pathToFileURL(path.join(previewDir, "homepage-minimal-topic-reel-preview.html")).href;
...
(async () => {
  ...
  await desktop.screenshot({
    path: path.join(outputDir, "homepage-minimal-topic-reel-desktop.png"),
    fullPage: true,
    animations: "disabled",
  });
  ...
  await mobile.screenshot({
    path: path.join(outputDir, "homepage-minimal-topic-reel-mobile.png"),
    fullPage: true,
    animations: "disabled",
  });

  await browser.close();
  console.log("Captured minimal homepage previews.");
})().catch((error) => {
  console.error(error);
  process.exit(1);
});
